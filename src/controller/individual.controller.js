...
const sendSupportRequest = async (req, res) => {
  try {
    ...
    const { issueDescription, preferredMethod } = req.body;
    if (!issueDescription || !preferredMethod) {
      return res.status(400).json({ error: 'issueDescription and preferredMethod are required' });
    }
...
const getSupportRequests = async (req, res, next) => {
  try {
    ...
    const requests = await Request.find({ user: req.user.id })
      .select('type issueDescription preferredMethod status submittedAt assignedAt completedAt')
      .sort({ submittedAt: -1 });
